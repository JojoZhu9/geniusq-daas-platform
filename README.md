...
真实 Key 只应保存在本地 `.env`，不要提交到 GitHub。DeepSeek 生成的 SQL 会先经过后端只读 SQL 校验，只有通过校验后才会查询本地 SQLite。
...
The real key should stay only in your local `.env` and must not be committed to GitHub. SQL generated by DeepSeek still goes through the backend read-only SQL guard before it can query the local SQLite database.
