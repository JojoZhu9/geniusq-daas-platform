...
- 历史会话自动保存，支持恢复单个会话、删除单条历史和清空全部历史；删除 / 清空前会二次确认，避免误操作。
...
- 测试 DeepSeek 连接，失败时区分缺少 Key、超时、鉴权失败、限流、网络异常和服务异常。
...
- Conversation history is saved automatically, with support for restoring one conversation, deleting one history item, and clearing all history; delete and clear actions require confirmation to avoid accidental loss.
